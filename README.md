...
<p align="center">
    ...
    <a href="https://jq.qq.com/?_wv=1027&k=8gnv52Vc"><img src="https://img.shields.io/badge/QQ群-870967170-brightgreen" /></a>
...
4. 回到`fuckTodayStudy`目录，将`依赖.zip`用压缩工具打开，全部解压到`fuckTodayStudy/今日校园`这个路径，也就是将依赖放到`今日校园`目录，
...
5. 回到函数计算网页，点击上传代码以上传压缩包（自行选择好您之前压缩的`xxx.zip`文件），然后点击`新建`即可，在这里您将等待一个漫长的漫长的漫长的时间...

6. 等待函数新建成功，进入到代码编辑，若您没有进行`本地测试`，请注意看这里，您现在可以返回到`本地测试`的第`4.5.6.10.11`步骤以完成您的`config.yml`的配置。若您之前进行了本地测试请您忽略本步骤

7. 点击`执行`按钮即可完成阿里云的函数部署，若报错请反馈报错代码截图（请确保本地运行正确先！）

8. 打开`触发器`，`创建触发器`，`服务类型：定时触发器`选择cron表达式，输入cron表达式如`0 0 7,12,14 * * *`代表7点、12点、14点各运行一次。自己输入`触发器名称`点击确定即可。

9. 点击提交即可完成自动化签到的部署了

10. ✈**Now, enjoy it please！**~~（子墨师兄的口头禅）~~✈
